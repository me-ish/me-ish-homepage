// api/natori/delivery/accept/route.ts
// 納品ページの「受け取りました」ボタンからの検収確定。
...
export async function POST(req: Request) {
  ...
  const rl = await checkRateLimit(`natori-delivery-accept:${ip}`, {
    limit: 10,
    windowMs: 600_000,
  });
  ...
  switch (result.kind) {
    ...
    case "expired":
      return NextResponse.json({ error: "delivery_expired" }, { status: 410 });
    case "db-error":
