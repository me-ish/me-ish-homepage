-- Align existing icon / sd / standing project tasks to the unified 6-step
-- production template. Illustration projects are intentionally left unchanged.
--
-- The app already creates new character projects with this template. This
-- migration rewrites saved rows for existing projects so task counts, labels,
-- estimated hours, and sort order match the current app behavior.
--
-- Labels use PostgreSQL Unicode escapes so this file stays ASCII.

with affected_projects as (
  select id, type, status
  from public.natori_projects
  where type in ('icon', 'sd', 'standing')
),
old_task_state as (
  select
    p.id as project_id,
    ...
    bool_or(t.task_key in ('review', 'finishing') and t.done) as review_done,
...
template_tasks as (
  ...
  cross join lateral (
    values
      (
        'rough',
        U&'\30E9\30D5\4F5C\6210',
        'rough',
        case p.type
          when 'icon' then 1::numeric
          when 'sd' then 1.5::numeric
          else 3::numeric
        end,
        0
      ),
      ...
      (
        ...
        'coloring',
        case p.type
          when 'icon' then 1.5::numeric
          when 'sd' then 4::numeric
          else 10::numeric
        end,
        3
      ),
      (
        'review',
        U&'\6700\7D42\78BA\8A8D',
        'finish',
        case p.type
          when 'icon' then 0.5::numeric
          else 1::numeric
        end,
        4
      ),
...
new_tasks as (
  select
    ...
    case
      ...
      when tt.task_key = 'review'
        then coalesce(ots.review_done, false)
...
where p.id = t.project_id
  and p.type in ('icon', 'sd', 'standing')
