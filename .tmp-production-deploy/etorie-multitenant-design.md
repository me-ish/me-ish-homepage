...
／ 2026-07-18（外部レビュー反映: service role のテナント境界を明文化・
制作/支払いフローのマイルストーン化 §12・Connect 工数の上方修正）
...
**着手時に再 grep すること（2026-07-18 外部レビュー指摘）**: 上表は 2026-07-14
時点の結果なのでその後のコードが漏れている。特に以下は取りこぼしやすい:
メールの replyTo / BCC のデフォルト値（NATORI_PORTFOLIO_CONTACT_TO /
NATORI_MAIL_BCC）、FROM 表示名、`X-Meish-Template` ヘッダの値、
`getSiteUrl()` と組み合わせた公開URL生成（承諾リンク・支払いリンクの文面埋め込み）。
...
**肥大化の予防（2026-07-18 追記）**: tenants に設定列を足し続けると
（カラー・テーマ・メール文面・通知設定…）巨大テーブルになる。上記の列
（識別・Stripe状態・受付状態）を超える設定類は `tenant_mail_settings` /
`tenant_theme_settings` のような 1:1 テーブルに分ける。目安は
「行の同時更新者が違うもの（本人設定 vs webhook 同期）を同居させない」。
...
**service role の鉄則（2026-07-18 外部レビュー指摘・P1 の受け入れ条件）**:
service role は RLS を素通りするため、行 ID 単独の UPDATE/DELETE は
「UUID を知っていれば他テナントの行を書ける」状態になる。P1 完了の定義に
以下を含める:

- service 層の**全ての UPDATE / DELETE / 単一行 SELECT は
  `.eq("tenant_id", tenant.id)` を必ず併記**する（`.eq("id", projectId)` 単独を
  禁止）。機械的に grep で検査できるよう、生の `.from("natori_...")` を散らさず
  tenant 必須のクエリヘルパ経由に寄せるのが望ましい。
- 例外は公開トークン系（見積もり承諾 `/api/natori/quote/accept` 等）。
  URL にテナント文脈が無いためトークンハッシュ単独で行を解決するのは正しいが、
  **解決した行から tenant_id を取り、以後の更新・通知・ログには必ずその
  tenant_id を引き回す**（別テナントの設定・メールアドレスを引かない）。
  トークン自体は 256bit ランダムなので推測耐性の問題はない。
...
- Webhook: connected accounts のイベントは **Connect webhook エンドポイント**
  ...
  逆引きで tenant が見つからない・`event.account` が無いイベントは
  **処理せず 200 で捨てて警告ログ**（解除済みアカウントの残イベント等。
  metadata の projectId だけを信じて書き込まない）。projectId と解決した
  tenant の不一致も同様に破棄対象（§5 の鉄則と同じ理由）。
...
- 公開URL: 旧 handle からの 301（旧 handle は一定期間予約して再取得を防ぐ。
  実装は `handle_history(tenant_id, old_handle, changed_at)` テーブルで
  旧 handle → tenant を引けるようにする。GitHub の username 変更と同じ考え方）
...
| P2 Connect | Express オンボーディング・direct charge 化（Checkout Session への乗り換え検討込み）・Connect webhook（dedup 込み）・アカウント状態ゲート・refund/dispute 通知・課金(Billing) | 10–14日 |
...
※ 2026-07-18: P2 を 7–10日 から上方修正。外部レビューでも「Connect は
設計書の見積もりより1段重い」と指摘。オンボーディング離脱・テスト用
connected account の準備・本番審査の待ち時間が読みにくい。
P0 は今の運用を壊さないので、暇なときに先行して進めてよい唯一のフェーズ。
なお P0〜P1 のスキーマ作業時に §12（マイルストーン）のテーブル形だけは
先に入れること。ここだけは後からの畳み直しが最も高くつく。
...
## 12. 制作フロー・支払いフローの一般化（2026-07-18 追記・外部レビュー起点）

**背景**: 現行 natori は「見積もり → 承諾 → **全額前払い** → ラフ → … → 納品」の
固定パイプライン（lib/statusTransitions.ts）+ 支払いリンク1本。だが実際の
イラストレーターは入金タイミングも工程順も人によって違う
（半額→ラフ→残額、ラフ後に全額、納品後払い 等）。フローを固定したまま
SaaS 化すると「自分のやり方と違う」が乗り換え拒否の筆頭理由になる。

**方針: 「案件状態」と「制作フロー」を分離する**

- **案件状態**（全テナント共通・少数固定）: 問い合わせ / 進行中 / 完了 / 見送り。
  一覧・集計・実績の軸はこちら。現行 statusTransitions の「前工程（inquiry〜
  awaiting_payment）」相当は問い合わせ管理の状態としてここに残る。
- **制作フロー**（テナント毎・案件毎）: 案件が**マイルストーン列**を持つ。
  例: ラフ提出 / ラフ承認 / 50%入金 / 清書 / 完成 / 残金入金 / 納品。
  順序は自由。現行の task checklist（stage 付きタスク）はマイルストーンの
  前身なので、データ移行は機械的に写像できる。
- **支払いはマイルストーンの属性**にする: マイルストーンに請求
  （金額または総額比率）を紐づけ、到達時に「50% を請求」ボタン →
  direct charge の Checkout Session（§6）→ 入金 webhook で該当
  マイルストーンを消し込む。現行の「支払いリンク1本 = 案件全額」は
  「入金マイルストーンが1個ある」の特殊ケースとして表現できる。

**UI は絞る（設定爆発の防止）**: 初期は支払いテンプレを3つだけ提供 —
①全額前払い ②半額→残額 ③納品後払い。テンプレ選択でマイルストーン列を
自動生成する。**内部表現は最初から全部マイルストーン**とし、カスタム並び替え
UI は要望が実際に積まれてから作る。

**段階投入**: 初期リリースは①（全額前払い = 現行フロー）だけ有効でも成立する。
ただし DB スキーマ（マイルストーンテーブル + 請求の紐づけ）は P0〜P1 の
時点でこの形にしておく（§9 注記）。承諾トークン・支払いリンクの発行/失効
ロジック（2026-07 実装済み）はマイルストーン請求単位にそのまま流用できる。
